...
//             $item['name']      =$info['filename'];             
    ...
    */
    ...
    <table cellpadding="0" cellspacing="1">
       ...
       <tfoot>
          <tr>
             <td class="total"><? if($folder_list): ?>This folder has <?=count($file_list)?> files totaling <?=$total_size['num']?> <?=$total_size['str']?> in size<?
    ...
    <? foreach($file_list as $item) : ?>
          <tr class="file">
             <td class="name"><img src="<?=getcwd().$this_script?>?image=<?=$item['ext']?>" alt="<?=strtolower($item['ext']);?>" /> <a href="<?=$item['name']?>.<?=$item['ext']?>"><?=$item

    ['name']?>.<?=$item['ext']?></a></td>
             <td class="size"><?=$item['size']['num']?><span><?=$item['size']['str']?></span></td>
             <td class="time"><?=time_ago($item['mtime'])?> ago</td>
          </tr>
    <? endforeach; ?>
    <? endif; ?>
    <!-- /files -->
